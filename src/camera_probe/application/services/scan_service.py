...
class ScanService:
    ...
    async def scan(
        ...
        *,
        ...
        try:
            network = ip_network(request.cidr, strict=False)
        except ValueError as exc:
            raise ValueError(f"invalid CIDR '{request.cidr}'") from exc
            ...
            while True:
                ...
                if on_result:
                    try:
                        ...
                        logger.exception("scan on_result callback failed | ip=%s", ip)
        ...
        try:
            ...
            tasks = [producer_task, *worker_tasks]
            for task in tasks:
                if not task.done():
                    task.cancel()
